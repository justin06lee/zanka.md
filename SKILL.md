...
- **Art direction: manga panel.** Banners and README illustrations are minimal black-and-white ink work — bold line art, flat black/white shapes, screentone dots or hatching for texture, speed lines for motion, a drawn panel border. No color, no gradients, no photorealism: the style must reduce to a handful of clean SVG paths. Wordmark/title text may sit inside or beside the panel.
- **Roll the aspect ratio — with a real RNG.** So banners don't converge on one shape, generate two random integers in **3–9** before drawing (`jot -r 2 3 9`, or `python3 -c 'import random;print(random.randint(3,9),random.randint(3,9))'`) — never "pick" the numbers yourself; model-chosen randomness clusters on the same values. Larger roll : smaller roll = width : height (equal rolls = square card), so every outcome lands between 1:1 and 3:1. Compose *for* the rolled shape instead of squeezing one layout into it — square-ish cards (display width ~300–380) suit a single tight panel with the wordmark under it; wide strips (display width ~560–680) suit a cinematic panel with speed-line room and the wordmark beside the art.
- **Find the reference before drawing.** In order: (1) distill the project into one visual motif; (2) check the user's existing repos (`gh repo list justin06lee --limit 100`) for related projects and prior banners — stay consistent with the family and don't reuse a manga reference already spent on another repo; (3) search for a manga panel or moment that *loosely* evokes the motif — the link can be a stretch as long as it's felt (a terminal multiplexer nodding to a swordsman drawing several blades at once; a sync daemon nodding to clones working in eerie unison); (4) draw an original SVG in manga-panel style riffing on it. If no direction feels right, download 2–4 candidate panels to a scratch dir, `open -a Preview` them, recommend one with reasoning, and draw after the user picks.
- **Reference, never reproduce.** Downloaded panels are inspiration only — never commit, trace, or embed a copyrighted scan in a repo. The shipped banner is always original ink-style vector art.
